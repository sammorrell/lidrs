...
impl PhotometricWeb {
    ...
    pub fn delta_angle(&self, i: usize) -> f64 {
        if self.is_spherically_symmetric() {
            ...
            match i {
                ...
                x if x >= self.planes.iter().count() - 1 => self.planes[i].angle() - self.planes[i - 1].angle(),
                _ => 0.5 * ( (self.planes[i].angle() - self.planes[i - 1].angle()) + (self.planes[i + 1].angle() - self.planes[i].angle()) ),
            }
        }
    }
    ...
    pub fn total_intensity(&self) -> f64 {
        self.planes.iter()
            .enumerate()
            .map(|(i, p)| {
                p.integrate_intensity() * self.delta_angle(i)
            })
            .sum()
    }
}
